...
router.post("/:projectId/threads", authRequired, async (req, res, next) => {
  try {
    ...
    // Get the thread with author details
    const { rows: threadWithAuthor } = await pool.query(
      `
      SELECT dt.id, dt.project_id, dt.title, dt.created_by, u.full_name AS author_name,
             u.email AS author_email, dt.created_at, dt.updated_at,
             0 AS message_count
      FROM discussion_threads dt
      JOIN users u ON u.id = dt.created_by
    ...
    res.status(201).json(threadWithAuthor[0]);
  } catch (err) {
    next(err);
  }
});
